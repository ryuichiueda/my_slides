...
* 手で打っているコマンドをファイルに書いて一気に実行できるようにしたもの．
    * プログラムでもある．
    * 我々がこれから書くのはbashのスクリプト（シェルのスクリプトなので，bashのスクリプトをシェルスクリプトと呼ぶ）<br />　
* ここからはおそらく一気に覚えられないので，
    * 見様見真似で書いて実行してみましょう．
    * 読めるようにはなってください．
...
* 不正な入力に対する挙動もテストするとよい 　　　　　
    ...
    * 特定の終了ステータスを返しているかどうか
	...
	[ "$res" = 0 ] && echo OK
        exit $res
        ```
        * 注意: 終了ステータスはコマンドを実行したらすぐ確認<br />（変わってしまうので．）
...
* まとめ
    ...
    * リグレッションテストの記述
        * プログラムと一緒に少しずつ書いていく
        * ROSのように標準入出力を使わないものについては第11回で<br />　
* 後始末
